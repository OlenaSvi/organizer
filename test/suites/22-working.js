...
T.head("«ОТЛОЖИТЬ» УНОСИТ ДЕЛО ИЗ ДНЯ ЦЕЛИКОМ");
/* Отложить — значит «сегодня делать не буду». Оставлять дело в дне
   после этого было бы противоречием: у дела в работе других выходов
   нет, «в предложения» у него убрана. */
clickOn({ act: "work", id: g.id });
T.ok("признак снят", !g.working);
T.ok("из дня ушло", !pickedToday(g));
T.ok("и сегодня больше не предлагается",
  !todayItems().proposals.some(function (x) { return x.it === g; }));
T.ok("нигде в дне его нет", todayItems().mine.indexOf(g) < 0);
closeModal();

T.head("У ДЕЛА В РАБОТЕ ДРУГИХ ВЫХОДОВ НЕТ");
/* Кнопка «в предложения» на нём противоречива: дело уходило из дня, а
   признак работы оставался — кружок говорил одно, экран другое. */
T.reset();
var q = T.tasks()[0];
q.due = null;
openItem(q.id);
clickOn({ act: "work", id: q.id });
T.ok("в окне нет «убрать из сегодня»",
  host.innerHTML.indexOf("Убрать из сегодня") < 0);
closeModal();
TAB = "today"; TODAYVIEW = "work"; render();
/* T.card отдаёт хвост колонки, поэтому по нему нельзя судить об
   отсутствии текста — смотрим кнопку именно этого дела. */
T.ok("у него нет кнопки ухода из дня",
  !new RegExp('data-act="unpick" data-id="' + q.id + '"').test(view.innerHTML));
T.ok("и добавления тоже",
  !new RegExp('data-act="pin" data-id="' + q.id + '"').test(view.innerHTML));
T.ok("галочка на месте — это выход", T.card(q.id).indexOf('data-act="toggle"') >= 0);
TODAYVIEW = "all";
